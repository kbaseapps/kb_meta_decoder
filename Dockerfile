...
RUN apt-get update && apt-get install -y bowtie samtools bcftools bedtools emboss python3-pip python3-numpy wget zlib1g-dev emacs bc gawk vcftools
# need newer bcftools than in repo for this old image:
RUN wget http://ftp.us.debian.org/debian/pool/main/h/htslib/libhts2_1.9-11_amd64.deb && wget http://ftp.us.debian.org/debian/pool/main/b/bcftools/bcftools_1.9-1_amd64.deb && wget http://ftp.us.debian.org/debian/pool/main/libd/libdeflate/libdeflate0_1.5-1_amd64.deb && dpkg --install *.deb
